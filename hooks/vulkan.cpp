...
VkResult queue_present_hook(VkQueue queue, const VkPresentInfoKHR* present_info) {
  ...
  for (uint32_t i = 0; i < present_info->swapchainCount; ++i) {
	...
	info.renderArea.extent.width = 3440;//resolution.x;
	info.renderArea.extent.height = 1440;//resolution.y;
      } else {
    ...
    if (!ImGui::GetIO().BackendRendererUserData) {
      ...
      ImGui::GetIO().DisplaySize.x = 3440;//resolution.x;
      ImGui::GetIO().DisplaySize.y = 1440;//resolution.y;
